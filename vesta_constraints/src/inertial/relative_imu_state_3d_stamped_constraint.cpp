...
RelativeImuState3DStampedConstraint::RelativeImuState3DStampedConstraint(
  ...
  double info_weight) :
    vesta_core::Constraint(
      source,
      {orientation1.uuid(), position1.uuid(), velocity1.uuid(),  // NOLINT(whitespace/braces)
       gyro_bias1.uuid(), accel_bias1.uuid(),
       orientation2.uuid(), position2.uuid(), velocity2.uuid(),
       gyro_bias2.uuid(), accel_bias2.uuid()}),
...
ceres::CostFunction* RelativeImuState3DStampedConstraint::costFunction() const
{
  return new ceres::AutoDiffCostFunction<NormalDeltaImuState3DCostFunctor, 15, 4, 3, 3, 3, 3, 4, 3, 3, 3, 3>(
    new NormalDeltaImuState3DCostFunctor(sqrt_information_, delta_q_, delta_p_, delta_v_, dt_,
                                          gravity_, linearization_bg_, linearization_ba_,
                                          dq_dbg_, dp_dbg_, dp_dba_, dv_dbg_, dv_dba_));
}
