...
#include <vesta_constraints/inertial/cost_functions/normal_delta_imu_state_3d_cost_functor.h>
#include <vesta_constraints/inertial/cost_functions/normal_delta_imu_state_3d_with_extrinsic_cost_functor.h>
...
RelativeImuState3DStampedConstraint::RelativeImuState3DStampedConstraint(
    ...
    const Eigen::Vector3d& linearization_bg, const Eigen::Vector3d& linearization_ba, const Eigen::Vector3d& gravity,
    double info_weight)
  : vesta_core::Constraint(source, { orientation1.uuid(), position1.uuid(),
                                     velocity1.uuid(),  // NOLINT(whitespace/braces)
                                     gyro_bias1.uuid(), accel_bias1.uuid(), orientation2.uuid(), position2.uuid(),
                                     velocity2.uuid(), gyro_bias2.uuid(), accel_bias2.uuid() })
  , delta_q_(preintegrator.delta.q)
  , delta_p_(preintegrator.delta.p)
  , delta_v_(preintegrator.delta.v)
  , dt_(preintegrator.delta.dt)
  ...
  , gravity_(gravity)
  , dq_dbg_(preintegrator.jacobian.dq_dbg)
...
RelativeImuState3DStampedConstraint::RelativeImuState3DStampedConstraint(
  ...
  : vesta_core::Constraint(source, { orientation1.uuid(), position1.uuid(),
                                     velocity1.uuid(),  // NOLINT(whitespace/braces)
                                     gyro_bias1.uuid(), accel_bias1.uuid(), orientation2.uuid(), position2.uuid(),
                                     velocity2.uuid(), gyro_bias2.uuid(), accel_bias2.uuid(), ext_position.uuid(),
...
void RelativeImuState3DStampedConstraint::print(std::ostream& stream) const
{
  ...
  if (has_extrinsic_)
  {
    stream << "  ext_position variable: " << variables().at(10) << "\n"
           << "  ext_orientation variable: " << variables().at(11) << "\n";
  }

  stream << "  dt: " << dt_ << "\n"
         << "  delta_q: [" << delta_q_.w() << ", " << delta_q_.x() << ", " << delta_q_.y() << ", " << delta_q_.z()
         << "]\n"
...
ceres::CostFunction* RelativeImuState3DStampedConstraint::costFunction() const
{
  ...
  return new ceres::AutoDiffCostFunction<NormalDeltaImuState3DCostFunctor, 15, 4, 3, 3, 3, 3, 4, 3, 3, 3, 3>(
      new NormalDeltaImuState3DCostFunctor(sqrt_information_, delta_q_, delta_p_, delta_v_, dt_, gravity_,
                                           linearization_bg_, linearization_ba_, dq_dbg_, dp_dbg_, dp_dba_, dv_dbg_,
                                           dv_dba_));
}

}  // namespace vesta_constraints

BOOST_CLASS_EXPORT_IMPLEMENT(vesta_constraints::RelativeImuState3DStampedConstraint);
