...
bool ImuPreintegrator::integrate(const vesta_core::Timestamp &t,
  ...
  for (auto iter = data.begin(); std::next(iter) != data.end(); ++iter) {
    ...
    const double dt_sec =
        static_cast<double>(next->first.nanoseconds - iter->first.nanoseconds) *
        1e-9;
    increment(dt_sec, iter->second, bg, ba, compute_jacobian,
              compute_covariance);
    last_iter = next;
  }
...
void ImuPreintegrator::computeSqrtInformation() {
  ...
  delta.sqrt_information = Eigen::LLT<Eigen::Matrix<double, ES_SIZE, ES_SIZE>>(
  ...
  if (!delta.sqrt_information.allFinite()) {
    ...
    delta.sqrt_information =
        Eigen::Matrix<double, ES_SIZE, ES_SIZE>::Identity() *
        invalid_sqrt_info_weight;
  }
}
