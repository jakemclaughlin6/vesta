...
void ImuPreintegrator::increment(double dt, const ImuData& imu, const Eigen::Vector3d& bg, const Eigen::Vector3d& ba,
                                 bool compute_jacobian, bool compute_covariance)
{
  ...
  if (compute_covariance)
  {
    ...
    A.block<3, 3>(ES_V, ES_Q) = -dt * delta.q.matrix() * skewSymmetric<double>(a);
    A.block<3, 3>(ES_P, ES_Q) = -0.5 * dt * dt * delta.q.matrix() * skewSymmetric<double>(a);
    A.block<3, 3>(ES_P, ES_V) = dt * Eigen::Matrix3d::Identity();

    Eigen::Matrix<double, 9, 6> B;
    B.setZero();
    B.block<3, 3>(ES_Q, ES_BG - ES_BG) = dt * rightJacobianSO3(w * dt);
    ...
    delta.covariance.block<9, 9>(ES_Q, ES_Q) =
        A * delta.covariance.block<9, 9>(0, 0) * A.transpose() + B * white_noise_cov * B.transpose();
  ...
  {
    jacobian.dp_dbg +=
        dt * jacobian.dv_dbg - 0.5 * dt * dt * delta.q.matrix() * skewSymmetric<double>(a) * jacobian.dq_dbg;
    jacobian.dp_dba += dt * jacobian.dv_dba - 0.5 * dt * dt * delta.q.matrix();
    jacobian.dv_dbg -= dt * delta.q.matrix() * skewSymmetric<double>(a) * jacobian.dq_dbg;
    jacobian.dv_dba -= dt * delta.q.matrix();
    jacobian.dq_dbg = q_full.conjugate().matrix() * jacobian.dq_dbg - dt * rightJacobianSO3(w * dt);
  }
...
bool ImuPreintegrator::integrate(const vesta_core::Timestamp& t, const Eigen::Vector3d& bg, const Eigen::Vector3d& ba,
                                 bool compute_jacobian, bool compute_covariance, bool compute_information)
{
  if (data.empty())
  {
    return false;
  ...
  for (auto iter = data.begin(); std::next(iter) != data.end(); ++iter)
  {
    ...
    const double dt_sec = static_cast<double>(next->first.nanoseconds - iter->first.nanoseconds) * 1e-9;
  ...
  const double dt_final = static_cast<double>(t.nanoseconds - last_iter->first.nanoseconds) * 1e-9;
  if (dt_final > 0.0)
  {
    increment(dt_final, last_iter->second, bg, ba, compute_jacobian, compute_covariance);
  }
...
void ImuPreintegrator::computeSqrtInformation()
{
  ...
  delta.sqrt_information =
      Eigen::LLT<Eigen::Matrix<double, ES_SIZE, ES_SIZE>>(delta.covariance.inverse()).matrixL().transpose();

  if (!delta.sqrt_information.allFinite())
  {
    // Fall back to low-weight diagonal
    delta.sqrt_information = Eigen::Matrix<double, ES_SIZE, ES_SIZE>::Identity() * invalid_sqrt_info_weight;
  }
}
