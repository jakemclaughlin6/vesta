...
void ImuPreintegration::setStart(const vesta_core::Timestamp& stamp, const Eigen::Quaterniond& orientation,
...
ImuState ImuPreintegration::predictState(const vesta_core::Timestamp& stamp) const
{
  ...
  temp_preintegrator.integrate(stamp, current_state_.gyro_bias, current_state_.accel_bias, false, false, false);
  ...
  Eigen::Vector3d v_new = current_state_.velocity + params_.gravity * dt + R_curr * temp_preintegrator.delta.v;

  // p_new = p_curr + v_curr * dt + 0.5 * g * dt^2 + R_curr * delta_p
  Eigen::Vector3d p_new = current_state_.position + current_state_.velocity * dt + 0.5 * params_.gravity * dt * dt +
...
ImuPreintegration::PreintegrationResult ImuPreintegration::createPreintegratedFactor(const vesta_core::Timestamp& stamp,
                                                                                     const vesta_core::UUID& device_id,
                                                                                     bool add_prior_on_first_window)
{
  ...
  if (first_window_ && add_prior_on_first_window)
  {
    ...
    mean << current_state_.orientation.w(), current_state_.orientation.x(), current_state_.orientation.y(),
        current_state_.orientation.z(), current_state_.position, current_state_.velocity, current_state_.gyro_bias,
        current_state_.accel_bias;

    result.prior_constraint = std::make_shared<AbsoluteImuState3DStampedConstraint>(
        params_.source, orientation_i_, position_i_, velocity_i_, gyro_bias_i_, accel_bias_i_, mean, prior_covariance);
  ...
  // information
  preintegrator_.integrate(stamp, current_state_.gyro_bias, current_state_.accel_bias, true, true, true);
  ...
  Eigen::Vector3d v_new = current_state_.velocity + params_.gravity * dt + R_curr * preintegrator_.delta.v;
  ...
  result.relative_constraint = std::make_shared<RelativeImuState3DStampedConstraint>(
      params_.source, orientation_i_, position_i_, velocity_i_, gyro_bias_i_, accel_bias_i_, *orientation_j,
      *position_j, *velocity_j, *gyro_bias_j, *accel_bias_j, preintegrator_, current_state_.gyro_bias,
