...
ImuPreintegration::ImuPreintegration(const ImuPreintegrationParams &params,
                                     double info_weight)
    : params_(params), info_weight_(info_weight) {
  if (params_.cov_prior_noise <= 0.0) {
    LOG(ERROR) << "Prior noise on IMU state must be greater than zero, value: "
               << params_.cov_prior_noise;
    throw std::invalid_argument(
        "ImuPreintegrationParams::cov_prior_noise must be > 0");
  }
  setupPreintegrator();
}
...
void ImuPreintegration::setStart(const vesta_core::Timestamp &stamp,
...
void ImuPreintegration::setBiases(const Eigen::Vector3d &gyro_bias,
...
ImuState
ImuPreintegration::predictState(const vesta_core::Timestamp &stamp) const {
  if (!started_) {
  ...
  temp_preintegrator.integrate(stamp, current_state_.gyro_bias,
                               current_state_.accel_bias, false, false, false);
  ...
  Eigen::Quaterniond q_new(R_curr *
                           temp_preintegrator.delta.q.toRotationMatrix());
  q_new.normalize();
  ...
  Eigen::Vector3d p_new =
      ...
      0.5 * params_.gravity * dt * dt + R_curr * temp_preintegrator.delta.p;
...
ImuPreintegration::createPreintegratedFactor(const vesta_core::Timestamp &stamp,
                                             const vesta_core::UUID &device_id,
                                             bool add_prior_on_first_window) {
  PreintegrationResult result;

  if (!started_) {
    LOG(ERROR) << "ImuPreintegration::createPreintegratedFactor called before "
                  "setStart()";
    throw std::runtime_error("ImuPreintegration::createPreintegratedFactor "
                             "called before setStart()");
  }

  if (preintegrator_.data.empty()) {
    LOG(WARNING)
        << "Cannot create preintegrated factor, no IMU data is available.";
    throw std::runtime_error(
        "Cannot create preintegrated factor, no IMU data is available.");
  }

  if (stamp < preintegrator_.data.begin()->first) {
    LOG(WARNING) << "Cannot create preintegrated factor, requested time is "
                    "prior to the front of the window.";
    throw std::runtime_error(
        "Requested time is prior to the front of the IMU data window.");
  }
  ...
  if (first_window_ && add_prior_on_first_window) {
    ...
    // Build the 16x1 mean vector: [qw, qx, qy, qz, px, py, pz, vx, vy, vz, bgx,
    // bgy, bgz, bax, bay, baz]
    Eigen::Matrix<double, 16, 1> mean;
    mean << current_state_.orientation.w(), current_state_.orientation.x(),
        current_state_.orientation.y(), current_state_.orientation.z(),
        current_state_.position, current_state_.velocity,
        current_state_.gyro_bias, current_state_.accel_bias;

    result.prior_constraint =
        std::make_shared<AbsoluteImuState3DStampedConstraint>(
            params_.source, orientation_i_, position_i_, velocity_i_,
            gyro_bias_i_, accel_bias_i_, mean, prior_covariance);

    first_window_ = false;
  }

  // Integrate between keyframes, computing jacobians, covariance, and sqrt
  // information
  preintegrator_.integrate(stamp, current_state_.gyro_bias,
                           current_state_.accel_bias, true, true, true);
  ...
  Eigen::Vector3d p_new =
      current_state_.position + current_state_.velocity * dt +
      0.5 * params_.gravity * dt * dt + R_curr * preintegrator_.delta.p;
  ...
  auto orientation_j =
      std::make_shared<vesta_variables::Orientation3DStamped>(stamp, device_id);
  ...
  auto velocity_j = std::make_shared<vesta_variables::VelocityLinear3DStamped>(
      stamp, device_id);
  velocity_j->x() = v_new.x();
  velocity_j->y() = v_new.y();
  velocity_j->z() = v_new.z();

  auto gyro_bias_j = std::make_shared<vesta_variables::GyroscopeBias3DStamped>(
      stamp, device_id);
  ...
  auto accel_bias_j =
      std::make_shared<vesta_variables::AccelerationBias3DStamped>(stamp,
                                                                   device_id);
  ...
  result.relative_constraint =
      std::make_shared<RelativeImuState3DStampedConstraint>(
          params_.source, orientation_i_, position_i_, velocity_i_,
          gyro_bias_i_, accel_bias_i_, *orientation_j, *position_j, *velocity_j,
          *gyro_bias_j, *accel_bias_j, preintegrator_, current_state_.gyro_bias,
          current_state_.accel_bias, params_.gravity, info_weight_);
