...
/**
 * @brief Implements a cost function that models the relative constraint between two 3D IMU states
 *        using preintegrated IMU measurements.
 *
 ...
 *
 * where the corrected preintegrated measurements account for bias changes from the linearization
 * point using first-order Jacobians.
 */
    ...
    T* residual) const
{
  ...
  const Eigen::Matrix<T, 3, 1> res_p =
      q_i.conjugate() * (p_j - p_i - dt * v_i - T(0.5) * dt * dt * G) - dp_corrected;
