 ...
 * two 3D IMU states using preintegrated IMU measurements.
 *
 * Based on the on-manifold IMU preintegration formulation from Forster et al.,
 * DOI: 10.15607/RSS.2015.XI.006.
 *
 * The residual is computed as:
 *
 *   res_q  = 2 * (dq_corrected^-1 * (q1^-1 * q2)).vec()
 *   res_p  = q1^T * (p2 - p1 - v1*dt - 0.5*dt^2*g) - dp_corrected
 *   res_v  = q1^T * (v2 - v1 - dt*g) - dv_corrected
 *   res_bg = bg2 - bg1
 *   res_ba = ba2 - ba1
 *
 *   residual = sqrt_information * [res_q; res_p; res_v; res_bg; res_ba]
 *
...
public:
   ...
   * @param[in] linearization_bg  Gyroscope bias at preintegration linearization
   * point (3D)
   * @param[in] linearization_ba  Accelerometer bias at preintegration
   * linearization point (3D)
   * @param[in] dq_dbg            Jacobian of preintegrated rotation w.r.t. gyro
   * bias (3x3)
   * @param[in] dp_dbg            Jacobian of preintegrated position w.r.t. gyro
   * bias (3x3)
   * @param[in] dp_dba            Jacobian of preintegrated position w.r.t.
   * accel bias (3x3)
   * @param[in] dv_dbg            Jacobian of preintegrated velocity w.r.t. gyro
   * bias (3x3)
   ...
   *
   * Parameter block ordering (10 blocks):
   *   orientation1[4], position1[3], velocity1[3], gyro_bias1[3],
   * accel_bias1[3], orientation2[4], position2[3], velocity2[3], gyro_bias2[3],
   * accel_bias2[3]
   *
