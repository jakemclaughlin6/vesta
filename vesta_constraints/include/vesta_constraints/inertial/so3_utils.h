...
inline Eigen::Matrix<T, 3, 3> expMapSO3(const Eigen::Matrix<T, 3, 1>& theta)
{
  ...
  {
    // Rodrigues formula: R = I + sin(a)/a * [theta]_x + (1-cos(a))/a^2 * [theta]_x^2
...
/// @details Used during preintegration for bias correction. Double-only since it
...
inline Eigen::Matrix3d rightJacobianSO3(const Eigen::Vector3d& theta)
{
  ...
  return Eigen::Matrix3d::Identity()
         - ((1.0 - std::cos(angle)) / angle_sq) * skew
         + ((angle - std::sin(angle)) / (angle_sq * angle)) * skew_sq;
}

/// @brief Small-angle quaternion from a rotation vector (first-order approximation).
/// @details Constructs q = [1, theta/2] and normalizes. This is the approximation
