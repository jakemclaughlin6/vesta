...
/**
 * @brief A constraint that represents the relative change between two 3D IMU states using
 *        preintegrated IMU measurements.
 *
 * This constraint connects two full IMU states (orientation, position, velocity, gyroscope bias,
 * and accelerometer bias) using preintegrated IMU measurements from the Forster et al. formulation.
 * The preintegrated measurements encode the relative motion between two keyframes while accounting
 * for bias corrections via first-order Jacobians.
 *
 * Variables order: ori1, pos1, vel1, bg1, ba1, ori2, pos2, vel2, bg2, ba2 (10 variables).
 */
...
public:
  ...
  /**
   * @brief Create a constraint from preintegrated IMU measurements between two IMU states
   *
  ...
  RelativeImuState3DStampedConstraint(
    ...
    const Eigen::Vector3d& gravity = kGravityWorld,
...
protected:
  ...
  Eigen::Matrix<double, 15, 15> sqrt_information_{Eigen::Matrix<double, 15, 15>::Zero()};  //!< Square root information matrix
  ...
  Eigen::Matrix3d dq_dbg_{Eigen::Matrix3d::Zero()};  //!< Jacobian of preintegrated rotation w.r.t. gyro bias
  Eigen::Matrix3d dp_dbg_{Eigen::Matrix3d::Zero()};  //!< Jacobian of preintegrated position w.r.t. gyro bias
  Eigen::Matrix3d dp_dba_{Eigen::Matrix3d::Zero()};  //!< Jacobian of preintegrated position w.r.t. accel bias
  Eigen::Matrix3d dv_dbg_{Eigen::Matrix3d::Zero()};  //!< Jacobian of preintegrated velocity w.r.t. gyro bias
  Eigen::Matrix3d dv_dba_{Eigen::Matrix3d::Zero()};  //!< Jacobian of preintegrated velocity w.r.t. accel bias
...
private:
  ...
  void serialize(Archive& archive, const unsigned int /* version */)
  {
    ...
    // Serialize quaternion components individually (Eigen internal order: x, y, z, w)
    archive & delta_q_.x();
    archive & delta_q_.y();
    archive & delta_q_.z();
    archive & delta_q_.w();
