...
using vesta_constraints::StereoNullspaceProjectionConstraint;
                              ...
                              double baseline)
{
  ...
  double v_l = fy * p_cam.y() / p_cam.z() + cy;
  double u_r = fx * (p_cam.x() - baseline) / p_cam.z() + cx;
  double v_r = fy * p_cam.y() / p_cam.z() + cy;
...
struct TestScene
{
  ...
  {
    std::vector<std::vector<Eigen::Vector4d>> obs(landmarks.size());
    for (size_t j = 0; j < landmarks.size(); ++j)
    {
      for (const auto& pose : poses)
      {
        obs[j].push_back(stereoProject(pose.position, pose.orientation, landmarks[j], fx, fy, cx, cy, baseline));
      }
    }
    return obs;
  }
...
TEST(StereoNullspaceProjectionConstraint, Constructor)
{
  TestScene scene;
  ...
  StereoCameraFixed calibration(0);
  calibration.fx() = scene.fx;
  calibration.fy() = scene.fy;
  calibration.cx() = scene.cx;
  calibration.cy() = scene.cy;
  calibration.baseline() = scene.baseline;

  vesta_core::Matrix4d cov = vesta_core::Matrix4d::Identity();

  EXPECT_NO_THROW(
      StereoNullspaceProjectionConstraint constraint("test", positions, orientations, calibration, obs[0], cov));
...
TEST(StereoNullspaceProjectionConstraint, ResidualAtGroundTruth)
{
  TestScene scene;
  auto obs = scene.generateObservations();
  ...
  const int residual_dim = 4 * static_cast<int>(scene.poses.size()) - 3;
...
TEST(StereoNullspaceProjectionConstraint, JacobianCheck)
{
  TestScene scene;
  auto obs = scene.generateObservations();
...
TEST(StereoNullspaceProjectionConstraint, Optimization)
{
  // Stereo provides scale observability through the baseline, so we expect
  // exact convergence to ground truth (unlike mono nullspace projection).
  TestScene scene;
  ...
  auto calibration = StereoCameraFixed::make_shared(0);
  calibration->fx() = scene.fx;
  calibration->fy() = scene.fy;
  calibration->cx() = scene.cx;
  calibration->cy() = scene.cy;
  ...
  // Add a stereo nullspace constraint for each landmark
  ...
  // Stereo has scale observability, so check exact convergence
  for (size_t i = 1; i < scene.poses.size(); ++i)
  {
    EXPECT_NEAR(scene.poses[i].position.x(), positions[i]->x(), 1e-3) << "Pose " << i << " position x";
    EXPECT_NEAR(scene.poses[i].position.y(), positions[i]->y(), 1e-3) << "Pose " << i << " position y";
    EXPECT_NEAR(scene.poses[i].position.z(), positions[i]->z(), 1e-3) << "Pose " << i << " position z";
  }
}

TEST(StereoNullspaceProjectionConstraint, ConvertFromStereoReprojection)
{
  TestScene scene;
  auto all_obs = scene.generateObservations();
  ...
  auto transaction = vesta_constraints::convertToStereoNullspaceConstraints("test", landmark_uuids, graph);
  ...
  // Should have one stereo nullspace constraint per landmark
  int nullspace_count = 0;
  for (const auto& constraint : graph.getConstraints())
  {
    if (dynamic_cast<const StereoNullspaceProjectionConstraint*>(&constraint))
    {
      ++nullspace_count;
    }
  }
  ...
  // No stereo reprojection constraints remain
  for (const auto& constraint : graph.getConstraints())
  {
    EXPECT_EQ(nullptr, dynamic_cast<const StereoReprojectionErrorConstraint*>(&constraint));
  }
}

TEST(StereoNullspaceProjectionConstraint, Serialization)
{
  TestScene scene;
  auto obs = scene.generateObservations();
  ...
  StereoNullspaceProjectionConstraint expected("test", positions, orientations, calibration, obs[0], cov);
  ...
  StereoNullspaceProjectionConstraint actual;
  {
  ...
  for (size_t i = 0; i < expected.numObservations(); ++i)
  {
    ...
    EXPECT_NEAR(expected.observations()[i][2], actual.observations()[i][2], 1e-9);
    EXPECT_NEAR(expected.observations()[i][3], actual.observations()[i][3], 1e-9);
  }

  for (int k = 0; k < 5; ++k)
  {
    EXPECT_NEAR(expected.calibration()[k], actual.calibration()[k], 1e-9);
  }
}
