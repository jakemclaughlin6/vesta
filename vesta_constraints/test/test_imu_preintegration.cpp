...
TEST(ImuPreintegration, BasicWorkflow)
{
  ...
  // Verify predicted state is reasonable (zero motion, so position/velocity
  // should be near zero)
...
TEST(ImuPreintegration, ConstantVelocityTrajectory)
{
  ...
  // Expected state2: p = [0.5, 0, 0], v = [1, 0, 0] (since gravity cancels in
  // world frame) In the full prediction: p = v0*dt + 0.5*g*dt^2 + R*delta_p
  ...
  // variables by UUID. We need to use the variables that were current BEFORE
  // the factor was created. The prior constraint references orientation_i_,
  // position_i_, etc. from BEFORE the call. But those internal variables have
  // been overwritten. The constraint stores UUIDs only. For testing, we need to
  // create fresh variables matching the UUIDs.

  // Actually, looking at the code more carefully, the prior was created with
  // the old orientation_i_ etc., and the relative constraint connects old state
  // to new state. The UUIDs in the constraints refer to the variable objects
  // that existed at creation time. We need to create a new set of variables for
  // state1 and use result.* for state2.
  ...
  // Verify state2 converged: expected position ~ [0.5, 0, 0], velocity ~ [1, 0,
  // 0]
...
TEST(ImuPreintegration, BiasEstimation)
{
  ...
  // IMU data: zero real angular velocity, but measured as bias (gyro reads bias
  ...
  // Add state1 param blocks and fix pose/velocity (we know the robot is
  // stationary)
  ...
  // We know state2 should also be stationary (identity orientation, zero pos,
  // zero vel)
  ...
  // With a short window and limited observability, we may not get exact
  // convergence, but the direction should be correct (gbias1.x moving
  // positive). Check that at least one of the bias states has moved in the
