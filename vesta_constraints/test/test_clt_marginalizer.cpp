...
TEST(CltMarginalizer, StructuralCorrectness)
{
  MixedGraph tg;
  tg.graph.optimize();

  vesta_constraints::CltMarginalizer ng(vesta_constraints::SparsityMode::Tree, false);
  auto txn = ng.marginalize("test", { tg.l1->uuid() }, tg.graph);
...
TEST(CltMarginalizer, ProducesPairwiseConstraints)
{
  MixedGraph tg;
  tg.graph.optimize();

  // Marginalize both landmarks — the SchurMarginalizer would create a single
  // dense constraint coupling p1 and p2, but CLT in Tree mode should
  // produce pairwise constraints (2 variables each), not independent single-variable priors
  vesta_constraints::CltMarginalizer ng(vesta_constraints::SparsityMode::Tree, false);
  auto txn = ng.marginalize("test", { tg.l1->uuid(), tg.l2->uuid() }, tg.graph);

  auto added_cons = txn.addedConstraints();
  for (auto it = added_cons.begin(); it != added_cons.end(); ++it)
  {
    EXPECT_LE(it->variables().size(), 2u) << "Each marginal constraint should involve at most 2 variables";
  }
}

TEST(CltMarginalizer, ReasonableApproximation)
{
  ...
  auto txn_ng = ng.marginalize("test", to_marginalize2, tg2.graph);

  tg1.graph.update(txn_schur);
  tg2.graph.update(txn_ng);
...
TEST(CltMarginalizer, MatchesSchur_SingleRemainingVariable)
{
  // When only one variable remains connected, no off-diagonals to approximate,
  ...
  auto txn_ng = ng.marginalize("test", { tg2.l2->uuid() }, tg2.graph);
...
TEST(CltMarginalizer, SubgraphMode)
{
  MixedGraph tg1;
  MixedGraph tg2;
  ...
  vesta_constraints::SchurMarginalizer schur(false);
  vesta_constraints::CltMarginalizer ng(vesta_constraints::SparsityMode::Subgraph, false);

  auto txn_schur = schur.marginalize("test", { tg1.l1->uuid(), tg1.l2->uuid() }, tg1.graph);
  auto txn_ng = ng.marginalize("test", { tg2.l1->uuid(), tg2.l2->uuid() }, tg2.graph);

  // Should produce a valid transaction with removed variables and added constraints
  auto removed_vars = txn_ng.removedVariables();
  auto removed_set = std::set<vesta_core::UUID>(removed_vars.begin(), removed_vars.end());
  EXPECT_EQ(removed_set.count(tg2.l1->uuid()), 1u);
  EXPECT_EQ(removed_set.count(tg2.l2->uuid()), 1u);

  auto added_cons = txn_ng.addedConstraints();
  EXPECT_GE(std::distance(added_cons.begin(), added_cons.end()), 1);

  // Graph should be optimizable after applying the transaction
  tg2.graph.update(txn_ng);
  tg2.graph.optimize();

  // Results should be reasonable vs SchurMarginalizer
  tg1.graph.update(txn_schur);
  tg1.graph.optimize();
...
TEST(CltMarginalizer, TreeVsSubgraph)
{
  MixedGraph tg_tree;
  MixedGraph tg_sub;
  MixedGraph tg_schur;

  tg_tree.graph.optimize();
  tg_sub.graph.optimize();
  tg_schur.graph.optimize();

  vesta_constraints::SchurMarginalizer schur(false);
  vesta_constraints::CltMarginalizer ng_tree(vesta_constraints::SparsityMode::Tree, false);
  vesta_constraints::CltMarginalizer ng_sub(vesta_constraints::SparsityMode::Subgraph, false);

  auto txn_schur = schur.marginalize("test", { tg_schur.l1->uuid(), tg_schur.l2->uuid() }, tg_schur.graph);
  auto txn_tree = ng_tree.marginalize("test", { tg_tree.l1->uuid(), tg_tree.l2->uuid() }, tg_tree.graph);
  auto txn_sub = ng_sub.marginalize("test", { tg_sub.l1->uuid(), tg_sub.l2->uuid() }, tg_sub.graph);

  tg_schur.graph.update(txn_schur);
  tg_tree.graph.update(txn_tree);
  tg_sub.graph.update(txn_sub);
  ...
  for (size_t i = 0; i < tg_schur.p1->size(); ++i)
  {
    EXPECT_NEAR(tg_schur.p1->data()[i], tg_tree.p1->data()[i], 0.5);
    EXPECT_NEAR(tg_schur.p1->data()[i], tg_sub.p1->data()[i], 0.5);
  }
  for (size_t i = 0; i < tg_schur.p2->size(); ++i)
  {
    EXPECT_NEAR(tg_schur.p2->data()[i], tg_tree.p2->data()[i], 0.5);
    EXPECT_NEAR(tg_schur.p2->data()[i], tg_sub.p2->data()[i], 0.5);
  }

  // Subgraph should generally be at least as good as Tree (closer to Schur)
  double tree_error = 0.0;
  double sub_error = 0.0;
  for (size_t i = 0; i < tg_schur.p1->size(); ++i)
  {
    tree_error += std::pow(tg_schur.p1->data()[i] - tg_tree.p1->data()[i], 2);
    sub_error += std::pow(tg_schur.p1->data()[i] - tg_sub.p1->data()[i], 2);
  }
  for (size_t i = 0; i < tg_schur.p2->size(); ++i)
  {
    tree_error += std::pow(tg_schur.p2->data()[i] - tg_tree.p2->data()[i], 2);
    sub_error += std::pow(tg_schur.p2->data()[i] - tg_sub.p2->data()[i], 2);
  }
  EXPECT_LE(sub_error, tree_error + 1.0e-6) << "Subgraph should be at least as good as Tree";
}

TEST(CltMarginalizer, FejFallbackMatchesNonFej)
{
  MixedGraph tg1;
  MixedGraph tg2;
  ...
  vesta_constraints::CltMarginalizer non_fej(vesta_constraints::SparsityMode::Tree, false);
  vesta_constraints::CltMarginalizer fej(vesta_constraints::SparsityMode::Tree, true);
...
TEST(CltMarginalizer, NoNonStampedVariables)
{
  // All-stamped path should work (delegates to QR)
  MixedGraph tg;
  tg.graph.optimize();

  vesta_constraints::CltMarginalizer ng(vesta_constraints::SparsityMode::Tree, false);
  auto txn = ng.marginalize("test", { tg.p1->uuid() }, tg.graph);
...
TEST(CltMarginalizer, RuntimeComparison)
{
  ...
  for (int i = 0; i < NUM_ITERATIONS; ++i)
  {
    ...
    tg.graph.optimize();
    vesta_constraints::CltMarginalizer ng(vesta_constraints::SparsityMode::Tree, false);
    ng.marginalize("test", { tg.l1->uuid(), tg.l2->uuid() }, tg.graph);
  }
  auto end_ng = std::chrono::high_resolution_clock::now();
  auto ng_us = std::chrono::duration_cast<std::chrono::microseconds>(end_ng - start_ng).count();
  ...
  std::cout << "[          ] BlockDiagonalMarginalizer:   " << bd_us / NUM_ITERATIONS << " us/marginalization"
            << std::endl;
  std::cout << "[          ] CltMarginalizer:              " << ng_us / NUM_ITERATIONS << " us/marginalization"
            << std::endl;
}
