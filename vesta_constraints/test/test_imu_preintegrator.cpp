...
TEST(ImuPreintegrator, ZeroMotion)
{
  ...
  bool success =
      preintegrator.integrate(vesta_core::Timestamp(static_cast<int64_t>(1.0 * 1e9)), bg, ba, true, true, true);
  ASSERT_TRUE(success);
...
TEST(ImuPreintegrator, CovarianceGrowth)
{
  ...
  preintegrator.integrate(vesta_core::Timestamp(static_cast<int64_t>(0.5 * 1e9)), bg, ba, false, true, false);
  double norm_half = preintegrator.delta.covariance.norm();

  // Integrate for 1.0 seconds
  preintegrator.integrate(vesta_core::Timestamp(static_cast<int64_t>(1.0 * 1e9)), bg, ba, false, true, false);
  double norm_full = preintegrator.delta.covariance.norm();

  // Integrate for 2.0 seconds
  preintegrator.integrate(vesta_core::Timestamp(static_cast<int64_t>(2.0 * 1e9)), bg, ba, false, true, false);
...
TEST(ImuPreintegrator, ResetClearsState)
{
  ...
  preintegrator.integrate(vesta_core::Timestamp(static_cast<int64_t>(0.5 * 1e9)), bg, ba, true, true, true);
