...
## Variable
Within the vesta system, a "variable" is a convenient group of one or more related scalar values. These scalars are
...
Within the `vesta_core` package, no concrete variables are actually created. We only define the common interface to which
all types must adhere. A set of common variable types are provided in the [`vesta_variables`](../vesta_variables)
package. And new variable types can be created outside the vesta stack completely. However, similar to how using common
