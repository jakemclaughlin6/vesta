...
static SchurTestResult
runVisualSlamWithSolver(ceres::LinearSolverType solver_type) {
  ...
  std::vector<Eigen::Vector3d> gt_landmarks = {
      {0.5, 0.5, 5.0},   {1.5, -0.5, 6.0},  {2.5, 0.3, 7.0},
      {3.5, -0.3, 5.5},  {0.0, 1.0, 8.0},   {1.0, -1.0, 6.5},
      {2.0, 0.0, 9.0},   {3.0, 0.5, 7.5},   {4.0, -0.5, 5.0},
      {2.0, 1.0, 10.0},
  };
  ...
  txn->stamp(vesta_core::Timestamp(
      static_cast<int64_t>(num_cams - 1) * 1000000000LL));
  ...
  // Prior on first pose (world position = 0 for camera at origin)
  {
    ...
    mean << gt_cam_world[0].x(), gt_cam_world[0].y(), gt_cam_world[0].z(), 1.0,
        0.0, 0.0, 0.0;
    ...
    auto prior = std::make_shared<
        ...
        "prior", *cam_positions[0], *cam_orientations[0], mean, cov);
  ...
  for (int i = 0; i < num_cams - 1; ++i) {
    ...
    auto rel = std::make_shared<
        ...
        "odom", *cam_positions[i], *cam_orientations[i],
        *cam_positions[i + 1], *cam_orientations[i + 1], delta, cov);
  ...
  for (int i = 0; i < num_cams; ++i) {
    for (int j = 0; j < num_lms; ++j) {
      ...
      if (u < 0 || u > 640 || v < 0 || v > 480) continue;
      ...
      auto c = std::make_shared<
          vesta_constraints::ReprojectionErrorConstraint>(
          "camera", *cam_positions[i], *cam_orientations[i], *cam_k,
          *landmarks[j], obs, cov);
      txn->addConstraint(c);
    }
  }
  ...
  for (int i = 0; i < num_cams; ++i) {
    const auto &pos = dynamic_cast<const vesta_variables::Position3DStamped &>(
        g.getVariable(cam_positions[i]->uuid()));
...
TEST(SchurSlam, DenseSchur) {
  ...
  for (int i = 0; i < 5; ++i) {
    EXPECT_NEAR(result.cam_positions[i].x(), static_cast<double>(i), 0.2)
        << "Cam " << i;
...
TEST(SchurSlam, IterativeSchur) {
  ...
  for (int i = 0; i < 5; ++i) {
    EXPECT_NEAR(result.cam_positions[i].x(), static_cast<double>(i), 0.3)
        << "Cam " << i;
