...
TEST(VisualInertialSlam, BatchWithStereoReprojection)
{
  ...
  for (int i = 0; i < num_keyframes; ++i)
  {
    ...
    EXPECT_NEAR(gb.x(), 0.0, 0.01) << "KF " << i << " gyro bias x";
    EXPECT_NEAR(gb.y(), 0.0, 0.01) << "KF " << i << " gyro bias y";
    EXPECT_NEAR(gb.z(), 0.0, 0.01) << "KF " << i << " gyro bias z";
  ...
  for (int j = 0; j < actual_num_landmarks; ++j)
  {
    ...
    EXPECT_NEAR(lm.x(), gt_landmarks[j].x(), 0.25) << "Landmark " << j << " x";
    EXPECT_NEAR(lm.y(), gt_landmarks[j].y(), 0.25) << "Landmark " << j << " y";
    EXPECT_NEAR(lm.z(), gt_landmarks[j].z(), 0.25) << "Landmark " << j << " z";
  }
}
